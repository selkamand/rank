...
assigns the *highest* ranks to specified values (in order), while all
remaining values share the same lower rank.  
[`reorder_by_priority()`](https://selkamand.github.io/rank/reference/reorder_by_priority.md)
uses those ranks to move priority values to the front of the vector.
...
computes a single combined rank across all columns of a data frame,
where each column is ranked within groups defined by all previous
columns. This produces a true hierarchical ordering.
...
See [CONTRIBUTING.md](https://selkamand.github.io/rank/CONTRIBUTING.md).
